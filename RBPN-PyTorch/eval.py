...
parser.add_argument('--file_list', type=str, default='tri_filelist.txt')
...
if opt.model_type == 'RBPN':
    model = RBPN(num_channels=1, base_filter=96,  feat = 48, num_stages=3, n_resblock=3, nFrames=opt.nFrames, scale_factor=opt.upscale_factor)
...
def eval():
    ...
    for batch in testing_data_loader:
        ...
        save_img_PIL(prediction.cpu().data, str(count), True)
...
def save_img_PIL(img, img_name, pred_flag): 
    pil_img = toPIL(img.squeeze().clamp(0,1))  
