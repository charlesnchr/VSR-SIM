...
def load_stack(filepath, nFrames, scale, other_dataset):
    tt = int(nFrames/2)
    stack = io.imread(filepath).astype('float') / 255
    target = stack[-1]
    input = stack[4]
    input = transform.rescale(input, 1/scale, order=3)

    target = Image.fromarray((target*255).astype('uint8'))

    # print('input skimage',input.min(),input.max())
    input = Image.fromarray((input*255).astype('uint8'))
    # print('input pil',np.array(input).min(),np.array(input).max())
    ...
    for i in seq:
        idx = 4 + i
        # frame = stack[idx]
        frame = transform.rescale(stack[idx], 1/scale, order=3)
        frame = Image.fromarray((frame*255).astype('uint8'))
        neigbor.append(frame)

    # torch.save((input,neigbor,target),'sample.pt')
...
def load_SIM_stack(filepath, nFrames, scale, other_dataset):
    tt = int(nFrames/2)
    stack = io.imread(filepath) / 255
    target = stack[-1]
    
    input = transform.rescale(target, 1/scale, order=3)
...
class DatasetFromFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        if self.future_frame:
            target, input, neigbor = load_stack(self.image_filenames[index], self.nFrames, self.upscale_factor, self.other_dataset)
        else:
...
class DatasetFromPreprocFolder(data.Dataset):
    def __init__(self, image_dir,nFrames, upscale_factor, data_augmentation, file_list, other_dataset, patch_size, future_frame, transform=None):
        super(DatasetFromPreprocFolder, self).__init__()
        alist = [line.rstrip() for line in open(join(image_dir,file_list))]
        self.image_filenames = [join(image_dir,x) for x in alist]
        self.nFrames = nFrames
        self.upscale_factor = upscale_factor
    ...
    def __getitem__(self, index):
        input,target,neigbor,flow,bicubic = torch.load(self.image_filenames[index])

        new_neigbor = []
        for n in neigbor:
            new_neigbor.append(n[0])

        new_flow = []
        for f in flow:
            new_flow.append(f[0])

        return input[0], target[0], new_neigbor, new_flow, bicubic[0]
